...
import { ensureCatalogRecoveredV2 } from "./lib/catalogRecoveryV2";
  ...
  if (!readyPromise) {
    readyPromise = pool
      ...
      .then(async (result) => {
        ...
        const catalogRecovery = await ensureCatalogRecoveredV2();
        await ensureCatalogSkuGuardV2();
        logger.info({ catalogRecovery }, "catalog recovery and SKU guard verified");
      })
      .catch((error) => {
...
export default async function handler(
  ...
  try {
    ...
    sendJson(res, 503, { error: "API database initialization failed.", detail });
    return;
  }
  ...
  try {
    ...
    const detail =
      error && typeof error === "object" && "message" in error
        ? String((error as Error).message).slice(0, 400)
        : String(error).slice(0, 400);
    sendJson(res, 500, { error: "API request failed during handling.", detail });
  }
}
