...
const MARKER = "catalog_post_normalization_2026_06_27_v3";
...
const EXPLICIT_DUPLICATES: Record<string, string> = {
...
async function normalizeCatalog(): Promise<Summary> {
  const marker = await pool.query("SELECT 1 FROM site_settings WHERE key = $1 LIMIT 1", [MARKER]);
  if (marker.rowCount) {
    const countResult = await pool.query<{ count: string }>("SELECT COUNT(*)::text AS count FROM products");
    const count = Number(countResult.rows[0]?.count ?? 0);
    return {
      skipped: true,
      productsBefore: count,
      productsAfter: count,
      servicePagesRemoved: 0,
      duplicatesRemoved: 0,
      productsUpdated: 0,
      variantGroupsAssigned: 0,
      dropdownProducts: 0,
    };
  }
  ...
  try {
    ...
    const productsBefore = rows.length;
    ...
    for (const [duplicateSku, canonical] of Object.entries(EXPLICIT_DUPLICATES)) {
      const duplicate = retained.find((row) => row.sku.toUpperCase() === duplicateSku);
      const keeper = retained.find((row) => row.sku.toUpperCase() === canonical);
      if (duplicate && keeper) {
    ...
    const liveRows = retained.filter((row) =>
      !Object.keys(EXPLICIT_DUPLICATES).includes(row.sku.toUpperCase()),
    );
    ...
    for (const group of groups.values()) {
      ...
      if (removed.length) {
        await client.query("DELETE FROM products WHERE id = ANY($1::int[])", [removed.map((row) => row.id)]);
    ...
    let dropdownProducts = 0;
    const uniqueRows = [...groups.values()].map((group) => group[0]);
    ...
    for (const row of uniqueRows) {
      ...
      productsUpdated++;
      if (variant) variantGroupsAssigned++;
      if (Object.values(mergedAttributes).some((values) => values.length > 1)) dropdownProducts++;
    }
    ...
    const productsAfter = Number(countResult.rows[0]?.count ?? 0);
