...
import { ProductCard, type ProductWithVariantCount } from "@/components/product-card";
...
function buildCategoryCollectionSchema(categoryName: string, canonicalPath: string, itemCount?: number) {
  return {
    "@context": "https://schema.org",
    "@type": "CollectionPage",
    name: categoryName,
    description: `Shop ${categoryName} replacement parts at All Window Door Parts.`,
...
function buildPageMeta(search: string, category: string) {
  if (category) {
    ...
    if (known) return { title: known.seoTitle, description: known.seoDescription };
    return {
      title: `${category} – Replacement Parts`,
      description: `Shop ${category} replacement parts at All Window Door Parts. Hard-to-find and OEM-specific hardware for every major brand. Veteran-owned, 40+ years experience. Shipping is determined at checkout.`,
    };
  }
  if (search) {
    return {
  ...
  return {
    ...
    description: "Browse 4,000+ in-stock window and door replacement parts. Shop casement operators, window balances, door locks, rollers, glazing seals, screen frames, and more. Veteran-owned. Shipping is determined at checkout.",
  };
}
...
export interface ProductBrowserProps {
  /** Path this page always self-canonicalizes to, regardless of search/sort/page refinements. */
  canonicalPath: string;
  /** When set, the category filter is pinned to this category (e.g. on /category/:slug) instead of being read from the query string. */
  lockedCategoryName?: string;
}

export function ProductBrowser({ canonicalPath, lockedCategoryName }: ProductBrowserProps) {
  const [, navigate] = useLocation();
  const rawSearch = useSearch();

  const urlParams   = new URLSearchParams(rawSearch);
  const urlSearch   = urlParams.get("search")   ?? "";
  const urlCategory = lockedCategoryName ?? (urlParams.get("category") ?? "");
...
setSuggestionsOpen(data.length > 0);
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    ...
    navigate(q ? `${canonicalPath}?search=${encodeURIComponent(q)}` : canonicalPath);
  };
  ...
  const structuredData = urlCategory
    ? [
        buildCategoryCollectionSchema(urlCategory, canonicalPath, productsData?.total),
        buildFAQSchema(buildCategoryFAQs(urlCategory)),
      ]
    : undefined;
  ...
  // In locked-category mode the sidebar/pills link between real category pages
  // instead of ?category= query params, and hide the current category from the list.
  const otherCategories = lockedCategoryName
    ? CATEGORIES.filter((c) => c.name !== lockedCategoryName)
    : CATEGORIES;
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 border-b pb-6">
            ...
            <div className="flex items-center gap-3 flex-wrap">
                    ...
                    {suggestions.map((s) => (
                      <button
                        ...
                        onMouseDown={() => {
                          setSearchInput(s);
                          setSuggestionsOpen(false);
                          navigate(`${canonicalPath}?search=${encodeURIComponent(s)}`);
                        }}
                      >
              ...
              {(urlSearch || (!lockedCategoryName && urlCategory) || hasActiveFilters) && (
            ...
            <>
              <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-5">
                {productsData.products.map((product) => {
                  return <ProductCard key={product.id} product={product as unknown as ProductWithVariantCount} />;
                })}
              </div>
